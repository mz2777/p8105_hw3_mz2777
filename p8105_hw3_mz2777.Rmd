...
**Comment:** the mean data value in different states mostly distribute between 15-30 from 2002 to 2010. The mean data value of state WV is lower than 15 in 2005 and 2009. The mean data value of state IN is lower than 15 in 2007.
...
**Description:** the size of resulting dataset is `r nrow(accel_data)` observations of `r ncol(accel_data)` variables which are `r names(accel_data[,1])`, `r names(accel_data[,2:5])` for the information of day, `r names(accel_data[,6])` for every minute in each day and `r names(accel_data[,7])`.

* Traditional analyses of accelerometer data focus on the total activity over the day. Using your tidied dataset, aggregate accross minutes to create a total activity variable for each day, and create a table showing these totals. Are any trends apparent?

There is no trend apparently based on the line plot. On Tuesday, Wednesday and Thursday, the variaty of activity counts is smaller than the other days.
...
**Conclusion**: On Wednesday and Tuesday, this patient's activity counts are lower relatively which may show that he did not do much activity on those two days, while on weekend, the activity counts is relatively high. In each day, we can see that this patients usually sleep after midnight and the sleeping time is about 5 hours which is very short. On Friday, he usually have high activity counts around 20:00 to 21:00. On weekends, the activity counts is high during 10:00 to 12:00.
